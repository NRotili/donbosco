            ...
            <table class="table table-striped">
                ...
                <tbody>
                    @foreach ($categorias as $categoria)
                        <tr>
                            <th @if ($categoria->status == 0) style="text-decoration:line-through" @endif>{{$categoria->id}}</th>
                            <td @if ($categoria->status == 0) style="text-decoration:line-through" @endif>{{$categoria->nombre}}</td>
                            <td width="10px">
                            ...
                            <td width="10px">
                                @can('panel.administracion.categorias.edit')
                                    <a href="{{route('panel.administracion.categorias.edit', $categoria)}}" class="@if (!$categoria->status) disabled @endif btn btn-xs btn-warning"><i class="fas fa-pen"></i></a>
                                    @endcan
                            </td>
                            <td width="10px">
                                @can('panel.administracion.categorias.destroy')
                                    <form action="{{route('panel.administracion.categorias.destroy', $categoria)}}" method="POST">
                                        @csrf
                                        @method('delete')
                                        <button
