    ...
    public function listahh()
    {
        ...
        foreach ($categorias as $categoria) {
            ...
            if ($categoria->productos()->count() > 0) {
                ...
                foreach ($categoria->productos()->orderBy('nombre')->get() as $producto) {
                    $fpdf->Cell(7, 5, "", 0, 0, 'C', 0);
    ...
    public function mayorista()
    {
        ...
        foreach ($categorias as $categoria) {
            ...
            if ($categoria->productos()->count() > 0) {
                ...
                $fpdf->Cell(27, 7, "", 0, 0, 'C', 0);
                $fpdf->Cell(140, 7, strtoupper($categoria->nombre), 0, 0, 'C', true);
                $fpdf->Ln(8);

                $fpdf->SetTextColor(0, 0, 0);
                $fpdf->SetFont('Arial', 'B', 10);
                $fpdf->Cell(27, 5, "", 0, 0, 'C', 0);
                $fpdf->Cell(100, 5, utf8_decode("Producto"), 0, 0, 'L');
                $fpdf->Cell(40, 5, utf8_decode("Precio Mayorista"), 0, 0, 'C');
                $fpdf->Ln(6);
                $fpdf->SetFont('Arial', '', 10);
                foreach ($categoria->productos()->orderBy('nombre')->get() as $producto) {
                    $fpdf->Cell(27, 5, "", 0, 0, 'C', 0);
                    ...
                    $fpdf->Cell(100, 5, $producto->nombre, 'T', 0, 'L', 0);
                    $fpdf->Cell(40, 5, $producto->preciomayorista, 'T', 0, 'C', 0);
                    $fpdf->Ln(6);
                }
                $fpdf->Ln(6);
